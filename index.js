...
/////////////////
const BotName = '∆stro Bot'; 
const instagram = 'https://instagram.com/shelby.dzn'; 
const whatsapp = 'wa.me/+48699545339'; 
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] Escaneie O QR-Code Com Seu WhatsApp!`);
});
...
conn.on('message-status-update', json =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] => ∆𝚂𝚃𝚁𝙾 𝙱𝙾𝚃`)
})
...
axios.get(`https://mhankbarbar.herokuapp.com/api/twit?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
	conn.sendMessage(id, '[ESPERE] Em andamento ⏳ aguarde um momento', MessageType.text)
    let hasil = `✅ Sucesso! por favor clique no link abaixo para baixar!🤖\nClique no link abaixo 📎\n\nTamanho: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://st4rz.herokuapp.com/api/tiktok?url=${tictoc}`).then((res) => {
	 conn.sendMessage(id, '[ESPERE] Em andamento ⏳ aguarde um momento', MessageType.text)
     let titoe = `✅ Sucesso !!! Clique no link abaixo para baixar!🤖\nClique no link abaixo 📎\n\nTítulo/Descrição: ${res.data.deskripsi} \n\nDuração: ${res.data.durasi}\n\nNome: ${res.data.nama}\n\nLink: ${res.data.urlvideo}`;
  ...
  if (messageType == 'imageMessage')
   {
       ...
       if (caption == '!scan')
       {
           ...
           readTextInImage(img)
               .then(data => {
                   console.log(data)
                   conn.sendMessage(id, `*😳Lendo a imagem...🤖* \n\nResultado: \n\n${data}`, MessageType.text);
               })
               .catch(err => {
                   console.log(err)
               })
   ...
   {
const get = require('got')
    const body = await get.post('https://api.kawalcorona.com/positif', {
    ...
    conn.sendMessage(id,`🌍QUANTIDADE DE CASOS GLOBAIS COVID-19🌐\n\n📍Positivo ==> ${positif} \n`, MessageType.text);
}
